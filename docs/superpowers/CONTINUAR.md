...
- **Despliegue real → guía propia en `docs/superpowers/DESPLIEGUE.md`** (creada 2026-07-27, es el documento de trabajo de este hito: su sección "Estado del despliegue" dice en qué paso se está). **Es el hito en curso elegido por el usuario.** Decisiones ya tomadas: Fase A con la URL `.vercel.app` para verificación técnica sin clientes reales, Fase B cuando compre el dominio; un solo entorno (sin staging); Supabase en la UE; BD de producción **vacía** (sin seed demo, sin variables `DEMO_*`); primer super-admin dado de alta a mano con SQL (no hay autoservicio). Preparación ya hecha en el repo: `prisma generate` explícito en el script `build` y script `db:deploy` (`a483b35`). Al hacerlo, recordar: replicar las variables de `.env.example` en producción (dando de alta un super-admin real en vez de depender de `DEMO_SUPERADMIN_*`), y que la migración `add_membership_invitation_completed_at` lleva un backfill que marca como completados a los dueños ya existentes — correcto en producción, pero conviene comprobarlo tras aplicarla.
